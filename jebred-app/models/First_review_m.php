...
class First_review_m extends CI_Model {
	...
	public function get_headline()
	{
		...
		else
		{
			...
			//$data['isi'] = "<label>".strip_tags($panjang[0])."...</label><a href='".site_url("review/$id")."'>Baca Selengkapnya</a>";
		}
		return $data;
	}
	...
	public function paging($p=1)
	{
		...
		$cfg['page'] = $p;
		$cfg['per_page'] = $this->setting->web_review_per_page;
		$cfg['num_rows'] = $jml;
	...
	private function paging_review_sql()
	{
		$this->db
			->from('review a')
			->join('user u', 'a.id_user = u.id', 'LEFT')
			->join('kategori_blog k', 'a.id_kategori = k.id', 'LEFT')
			->where('a.enabled', 1)
			->where('a.headline <>', 1)
	...
	public function review_show($offset, $limit)
	{
		$this->paging_review_sql();
		$data = $this->db
	...
	private function sterilkan_review(&$data)
	{
	...
	{
		// review agenda (kategori=1000) tidak ditampilkan
		$this->db
		...
		$this->db->limit(7);
		$data = $this->db->get('review a')->result_array();
	...
	{
		$sql = "SELECT COUNT(a.id) AS id FROM review a LEFT JOIN user u ON a.id_user = u.id LEFT JOIN kategori_blog k ON a.id_kategori = k.id WHERE a.enabled=1 AND a.tgl_upload < NOW()";
		$query = $this->db->query($sql);
		$row = $query->row_array();
		$jml_data = $row['id'];
	...
	public function full_arsip($offset=0, $limit=50)
	{
		...
		$sql = "SELECT a.*,u.nama AS owner,k.kategori, YEAR(tgl_upload) as thn, MONTH(tgl_upload) as bln, DAY(tgl_upload) as hri FROM review a LEFT JOIN user u ON a.id_user = u.id LEFT JOIN kategori_blog k ON a.id_kategori = k.id WHERE a.enabled=?
		...
		if ($query->num_rows()>0)
		{
			for ($i=0; $i<count($data); $i++)
			{
				...
				$data[$i]['no'] = $nomer;
				$data[$i]['tgl'] = $tgl;
				$data[$i]['isi'] = "<a href='".site_url("review/$id")."'>".$data[$i]['judul']."</a>, <i class=\"fa fa-user\"></i> ".$data[$i]['owner'];
			}
		}
		else
		{
	...
	private function sql_gambar_slide_review($gambar)
	{
		$this->db
	...
	// Jika $gambar_utama, hanya tampilkan gambar utama masing2 review terbaru
	public function slide_review($gambar_utama=FALSE)
	{
		$sql = [];
		$sql[] = $this->sql_gambar_slide_review('gambar');
		if (!$gambar_utama)
		{
			$sql[] = $this->sql_gambar_slide_review('gambar1');
			$sql[] = '('.$this->sql_gambar_slide_review('gambar2').')';
			$sql[] = '('.$this->sql_gambar_slide_review('gambar3').')';
		}
		$sql = implode('
		UNION
		', $sql);
	...
	public function slider_gambar()
	{
		...
		{
			case '1':
				# 10 gambar utama semua review terbaru
				$slider_gambar['gambar'] = $this->db
					...
					->limit(10)->get('review')->result_array();
				$slider_gambar['lokasi'] = LOKASI_FOTO_ARTIKEL;
				break;

			case '2':
				# 10 gambar utama review terbaru yang masuk ke slider atas
				$slider_gambar['gambar'] = $this->slide_review(true);
				$slider_gambar['lokasi'] = LOKASI_FOTO_ARTIKEL;
				break;
	...
	public function agenda_show($type = '')
	{
		$this->db
			...
			->join('review a', 'a.id = g.id_review', 'LEFT')
			->where('a.enabled', 1)
	...
	public function komentar_show()
	{
		$sql = "SELECT a.*, b.*, YEAR(b.tgl_upload) AS thn, MONTH(b.tgl_upload) AS bln, DAY(b.tgl_upload) AS hri, b.slug as slug
			FROM komentar a
			INNER JOIN review b ON a.id_review = b.id
			WHERE a.status = ? AND a.id_review <> 775
		...
		for ($i=0; $i<count($data); $i++)
		{
			...
			$data[$i]['komentar'] = "".$panjang[0]."...<a href='".site_url("review/".$data[$i]['thn']."/".$data[$i]['bln']."/".$data[$i]['hri']."/".$data[$i]['slug']." ")."'>baca selengkapnya</a>";
		}
	...
	public function get_kategori($id = 0)
	{
		...
		if (empty($data))
		{
			$judul = [
				999 => "Halaman Statis",
				1000 => "Agenda",
				1001 => "review Keuangan",
				$id => "review Kategori $id"
			];
	...
	public function get_review($url)
	{
		$this->db->select('a.*, u.nama AS owner, k.kategori_blog, k.slug AS kat_slug, YEAR(tgl_upload) AS thn, MONTH(tgl_upload) AS bln, DAY(tgl_upload) AS hri')
			->from('review a')
			->join('user u', 'a.id_user = u.id', 'left')
			->join('kategori_blog k', 'a.id_kategori = k.id', 'left')
			->where('a.enabled', 1)
	...
	{
		$data = $this->db->where('id_review', $id)
	...
	// Query sama untuk paging and ambil daftar review menurut kategori
	private function list_review_sql($id)
	{
		$this->db
			->from('review a')
			->join('user u', 'a.id_user = u.id', 'left')
	...
	public function list_review($offset = 0, $limit = 50, $id = 0)
	{
		$this->list_review_sql($id);
	...
	public function hit($url)
	{
		...
		if (in_array($id, $_SESSION['review']) OR $this->agent->is_robot() OR crawler() === TRUE) return;
	...
	{
		return $this->db->select('slug, YEAR(tgl_upload) AS thn, MONTH(tgl_upload) AS bln, DAY(tgl_upload) AS hri')->where(array('id' => $id))->get('review')->row_array();
	}
}
