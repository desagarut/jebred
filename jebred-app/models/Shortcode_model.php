...
class Shortcode_model extends CI_Model {
  ...
  public function __construct()
  {
   ...
   // $this->load->model('pamong_model');
	...
	private function grafik_rp_apbd($type, $thn)
	{
		...
		ob_start();
			include("sisiteras-app/views/keuangan/grafik_rp_apbd_chart.php");
		$res = ob_get_clean();
		return $res;
	}
	...
	private function tabel_rp_apbd($type, $thn, $smt1)
	{
		...
		ob_start();
			include("sisiteras-app/views/keuangan/tabel_laporan_rp_apbd_artikel.php");
		$output = ob_get_clean();
		return $output;
	}
  ...
  private function penerima_bantuan_penduduk_grafik($stat=0, $tipe=0)
    ...
    $heading = 'Penerima Bantuan (Penduduk)';
		...
		ob_start();
			include("sisiteras-app/views/statistik/penduduk_grafik_web.php");
		$res = ob_get_clean();
		return $res;
	}
  ...
  private function penerima_bantuan_penduduk_daftar($stat=0, $tipe=0)
	{
		...
		ob_start();
			include("sisiteras-app/views/statistik/peserta_bantuan.php");
		$res = ob_get_clean();
		return $res;
	}
  ...
  private function sotk_w_bpd()
    ...
    $desa = $this->config_model->get_data();
		...
		ob_start();
			include("sisiteras-app/views/beranda/bagan_sisip.php");
		$res = ob_get_clean();
		return $res;
	}
